<template>
  <div>
    ...
    <div class="flex items-center justify-end mb-6">
      <Link class="btn-indigo float-right" :href="route('admin.product2.create')">
      <span>Create</span>
      ...
      <table class="w-full whitespace-nowrap">
        ...
        <tbody>
          <tr v-for="product in products.data" :key="product.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
            ...
            <td class="border px-2.5 py-2">
              <Link class="inline-block" :href="route('admin.product2.edit', product.id)">
                <icon name="edit" class="mr-2 w-7 h-7" />
              </Link>
              <button type="button" @click="deleteData(route('admin.product2.delete', product.id))" class="w-fit">
